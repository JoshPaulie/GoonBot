...
def get_voice_line(attacker, victim, result):
    ...
    if result == 'winning':
        with open("files_assets/mugging_voice_lines/winning_voice_lines.txt") as f:
            voice_lines = f.readlines()

    elif result == 'losing':
        with open("files_assets/mugging_voice_lines/losing_voice_lines.txt") as f:
            voice_lines = f.readlines()

    elif result == 'losing_badly':
        with open("files_assets/mugging_voice_lines/losing_badly_voice_lines.txt") as f:
            voice_lines = f.readlines()
...
class MuggingGame(commands.Cog):
    ...
    async def mugError(self, ctx, error):
        if isinstance(error, commands.CommandOnCooldown):
            ...
            embed = discord.Embed(title=f"{ctx.author.name}, you're on cooldown!",
                                  description=f"{calc_time(error.retry_after)} remaining. **{end_timestamp.strftime('%H:%M %p')}**")
